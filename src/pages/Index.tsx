...
import studioLoop from "@/assets/studio-loop.mp4.asset.json";
import ctaLoop from "@/assets/cta-loop.mp4.asset.json";
...
import { SpectrogramCanvas } from "@/components/motion/SpectrogramCanvas";
import { AnimatedWaveSVG } from "@/components/motion/AnimatedWaveSVG";
...
function HeroDemo() {
  ...
  const play = async () => {
    ...
    if (audioRef.current.src) { audioRef.current.play(); setPlaying(true); return; }
    setBusy(true);
    try {
  ...
  return (
    <div className="border hairline bg-card/60 backdrop-blur-md rounded-xl p-6 md:p-8 max-w-2xl mx-auto relative overflow-hidden">
      ...
      <LiveWaveform audio={audioRef.current} height={56} className="mb-5 opacity-90" />
...
const FEATURES = [
  ...
  { icon: Sparkles, title: "Karaoke captions", desc: "Word-level highlighting in real time. Download SRT/VTT subtitles in a click." },
    ...
    <>
      ...
      <section className="relative overflow-hidden border-b hairline">
        <div className="absolute inset-0 z-0">
          <VideoBackdrop src={heroLoop.url} poster={heroWaveform} opacity={0.65} />
        ...
        <div className="relative z-10 mx-auto max-w-[1400px] px-6 pt-28 md:pt-40 pb-20 md:pb-32">
          ...
          >
            <span className="signal-dot" /> v2.1 · Motion release · 412 voices online
          </motion.div>
          ...
          <div className="flex flex-wrap gap-3 mb-20">
            <Link to="/playground" className="inline-flex items-center gap-2 px-6 py-3 bg-signal text-accent-foreground rounded-md font-medium hover:opacity-90 transition glow-signal">
              Open the studio <ArrowRight className="w-4 h-4" />
            </Link>
            <Link to="/docs" className="inline-flex items-center gap-2 px-6 py-3 border hairline rounded-md hover:bg-secondary transition text-sm">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-px bg-border">
          {FEATURES.map((f, i) => (
            <motion.div
              ...
              className="bg-background p-8 hover:bg-card transition group relative overflow-hidden"
            >
              ...
              <AnimatedWaveSVG className="absolute -bottom-1 left-0 right-0 h-8 opacity-0 group-hover:opacity-60 transition" />
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="border-y hairline bg-card/30 py-24 md:py-32 relative overflow-hidden">
        <div className="absolute right-0 top-0 bottom-0 w-1/2 hidden lg:block opacity-50">
          <SpectrogramCanvas className="h-full" />
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-px bg-border">
          {VOICES_FEATURED.map((v, i) => (
            <motion.div
              ...
              className="bg-background group cursor-pointer relative"
            >
              <div className="aspect-[4/5] overflow-hidden bg-card relative">
                ...
                <AnimatedWaveSVG className="absolute bottom-4 left-4 right-4 h-10 opacity-70 group-hover:opacity-100 transition" />
              </div>
              <div className="p-5">
        ...
        <div className="grid lg:grid-cols-2">
          <div className="p-10 md:p-20 relative z-10">
            ...
            <div className="bg-card border hairline rounded-lg p-5 font-mono text-[12.5px] mb-6 overflow-x-auto">
              ...
              <div className="mt-3 text-muted-foreground">// 3. Multi-voice dialogue</div>
              <div>vocalis.dialogue([{`{voice:"aria", text:"Hi"}, {voice:"hugo", text:"Salut"}`}])</div>
            </div>
          ...
          <div className="relative min-h-[400px] lg:min-h-full">
            <VideoBackdrop src={studioLoop.url} poster={studioMic} opacity={0.85} />
      ...
      <section className="border-t hairline py-32 text-center relative overflow-hidden">
        <VideoBackdrop src={ctaLoop.url} opacity={0.4} />
